...
LLLLLLLLLL
L.LLLLLL.L
L.LLLLL.LL""".lines().map { it.toCharArray().toMutableList() }.toList()
        private val input = """LLLLLLLLLL.LLLL.LLLLLLL.LLLLL.LLLLLLLLLLLLLLLLLLLLLL.LL.LLLLLLLLLLL.LLLLLL.LL.LLLL.LLLLLL.LLLLLLL
...
LLLLLLLLLLLLLLL.LLLLLLL.LLLLL.LLLLL.LLLLL.LLLLLL.L.LLLL.LLLLLLLL.LLLLLLLLLLLLLLLLL.LLL.LL.LLLLLL.""".lines().map { it.toCharArray().toMutableList() }.toList()
    }
    ...
    fun run(): Int {
        ...
        while (true) {
            ...
            input.forEachIndexed { ri, row ->
                for (i in row.indices) {
                    val checkNeighbours = checkNeighbours(i, ri, input)

                    if (input[ri][i] == '#') {
                        if (checkNeighbours.second >= 4) {
                            next[ri][i] = 'L'
                            changed = true
                        }
                    } else if (input[ri][i] == 'L') {
                        if (checkNeighbours.first) {
                            next[ri][i] = '#'
                            changed = true
                        }
                    }
        ...
        return input.map { row -> row.count { it == '#' } }.sum()
    }

    private fun checkNeighbours(x: Int, y: Int, seats: List<MutableList<Char>>): Pair<Boolean, Int> {
        var free = true
        var occupied = 0

        val dx = listOf(-1, 1, -1, 0, 1, -1, 0, 1)
        val dy = listOf(0, 0, -1, -1, -1, 1, 1, 1)

        for (j in dx.indices) {
            try {
                if (seats[y + dy[j]][x + dx[j]] == '#') {
                    free = false
                    occupied++
                }
            } catch (e : Exception) { /* ignore any out of bounds ex */ }
        }
        return Pair(free, occupied);
    }
    ...
    fun run2(): Int {
        ...
        while (true) {
            ...
            input.forEachIndexed { ri, row ->
                for (i in row.indices) {
                    val checkNeighbours = checkNeighbours2(i, ri, input)

                    if (input[ri][i] == '#') {
                        if (checkNeighbours.second >= 5) {
                            next[ri][i] = 'L'
                            changed = true
                        }
                    } else if (input[ri][i] == 'L') {
    ...
    private fun checkNeighbours2(x: Int, y: Int, seats: List<MutableList<Char>>): Pair<Boolean, Int> {
        var free = true
        var occupied = 0
...
fun main() {
    ...
    println("Part 1: ${Day11().run()}")
    println("Part 2: ${Day11().run2()}")
}