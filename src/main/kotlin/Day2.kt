class Day2 {
    companion object {
        private val passwords : List<String> = listOf(
            "9-10 m: mmmmnxmmmwm",
            "6-8 w: wpwwhxnv",
            "4-6 n: trwpnnnvq",
    ...
    fun run() : Int {
        ...
        passwords.forEach { entry ->
            ...
            val minCount = policyParts[0].split("-")[0].toInt()
            val maxCount = policyParts[0].split("-")[1].toInt()

            if (compare1(password, minCount, maxCount, policyParts[1].single())) correctPassCount++
        }

        return correctPassCount
    }

    private fun compare1(password: String, minCount: Int, maxCount: Int, char: Char) : Boolean {
        val count = countOccurrences(password, char)
    ...
    private fun countOccurrences(pass: String, char: Char): Int {
        return pass.filter { it == char }.count()
    }
    ...
    fun run2() : Int {
        ...
        passwords.forEach { entry ->
            ...
            val policyParts = policy.split(" ")
            val char = policyParts[1].single()
            val pos1 = policyParts[0].split("-")[0].toInt()
            val pos2 = policyParts[0].split("-")[1].toInt()

            if (compare2(password, pos1, pos2, char)) correctPassCount++
        }

        return correctPassCount
    }

    private fun compare2(password: String, pos1: Int, pos2: Int, char: Char) : Boolean {
...
fun main() {
    ...
    println("Part 1: ${Day2().run()}")
    println("Part 2: ${Day2().run2()}")
}