class Day6 {
    companion object {
        private const val answers : String = """ltbriofs
bitolfsr
olitfrbs
sbirloft
sbrftiol
...
zdjuxw"""
        ...
        answers.split("\n\n").forEach { group ->
            ...
            group.lines().forEach { line ->
                for (i in line.indices) {
                    if (!s.contains(line[i])) s.add(line[i])
                }
            }
            sum += s.size
        }
    ...
    fun run2() : Int {
        ...
        answers.split("\n\n").forEach { group ->
            ...
            group.lines().forEach { line ->
                line.toCharArray().forEach { s[it] = s.getOrDefault(it, 0) + 1 }
            }
            val filter = s.filter { it.value == group.split("\n").size }
            sum += filter.size
        }
        return sum
    }
...
fun main() {
    ...
    println("Part 1: ${Day6().run()}")
    println("Part 2: ${Day6().run2()}")
}