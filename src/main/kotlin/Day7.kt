class Day7 {
    companion object {
        private const val input = """dark orange bags contain 3 dark chartreuse bags.
...
bright fuchsia bags contain 1 muted cyan bag."""
        private const val LINES = """(.+) bags contain (.+)"""
        private const val BAGS = """(\d+) (.+?) bags?[,.]"""
    }

    fun run() : Int {
        val bags = mapInput()
        return bagsNeeded(bags, "shiny gold").size - 1
    }
    ...
    private fun bagsNeeded(bags: Map<String, List<Pair<Int, String>>>, s: String): List<String> {
        val options = bags.filterValues { (_, innerBag) -> innerBag.second == s }.map { it.key }.flatMap { bagsNeeded(bags, it) }.toMutableList()
        options.add(s)
        return options.distinct().toList()
    }

    private fun bagsContained(bags: Map<String, List<Pair<Int, String>>>, s: String): Int =
        if (bags.containsKey(s)) 1 + bags[s]!!.sumBy { it.first * bagsContained(bags, it.second) } else 1

    // Create a Map as: OuterBag : List<Count, InnerBag>
    private fun mapInput() = input.lines().map { line ->
        val (bag, content) = LINES.toRegex().find(line)!!.destructured
        val contentList = BAGS.toRegex().findAll(content).map {
...
fun main() {
    ...
    println("Part 1: ${Day7().run()}")
    println("Part 2: ${Day7().run2()}")
}